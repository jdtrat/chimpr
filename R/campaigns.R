...
ChmpCampaigns <- R6::R6Class(
  ...
  public = list(
    ...
    all = function(fields = NULL, exclude_fields = NULL, count = 10, 
      ...
      since_send_time = NULL, before_create_time = NULL, since_create_time = NULL, 
      list_id = NULL, folder_id = NULL, sort_field = NULL, 
      sort_dir = NULL, key = NULL, parse = TRUE, ...) {

      args <- ct(list(fields = fields, exclude_fields = exclude_fields, count = count,
        offset = offset, type = type, status = status, before_send_time = before_send_time, 
        since_send_time = since_send_time, before_create_time = before_create_time, 
        since_create_time = since_create_time, list_id = list_id, folder_id = folder_id, 
    ...
    content = function(fields = NULL, exclude_fields = NULL,
      ...
      if (is.null(self$id)) stop("must initialize the class with 'id' for this method")
      args <- ct(list(fields = fields, exclude_fields = exclude_fields))
      private$get(file.path("campaigns", self$id, "content"), args, parse, key, ...)
    },

    feedback = function(feedback_id = NULL, fields = NULL, exclude_fields = NULL,
        key = NULL, parse = TRUE, ...) {
      if (is.null(self$id)) stop("must initialize the class with 'id' for this method")
      args <- ct(list(fields = fields, exclude_fields = exclude_fields))
      part <- if (is.null(feedback_id)) "feedback" else file.path("feedback", feedback_id)
    ...
    send_checklist = function(fields = NULL, exclude_fields = NULL,
      ...
      if (is.null(self$id)) stop("must initialize the class with 'id' for this method")
      args <- ct(list(fields = fields, exclude_fields = exclude_fields))
      private$get(file.path("campaigns", self$id, "send-checklist"), args, parse, key, ...)
    }
  ),
